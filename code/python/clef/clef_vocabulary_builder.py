...
def build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf):    
    ...
    from clef_globals import max_df
...
def build_all_unigrams_vocabulary(corpus,vocabulary_src):    
    ...
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_unigrams'.format(vocabulary_src)
...
def build_raw_unigrams_vocabulary(corpus,vocabulary_src):
    ...
    # save to DB
    tbl_name = 'clef_2010_{0}_unigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_lemmatized_unigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_lemmas_unigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_lemmatized_test_unigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_lemmas_test_unigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_stemmed_test_unigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_stems_test_unigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_lemmatized_all_unigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_lemmas_all_unigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_stemmed_all_unigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_stems_all_unigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_unigrams_stopwords_vocabulary(corpus,stop_words,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_unigrams_stopwords_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_bigrams_stopwords_vocabulary(corpus,stop_words,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_bigrams_stopwords_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name


# build all unigrams stopwords vocabulary
def build_all_unigrams_stopwords_vocabulary(corpus,stop_words,vocabulary_src):
    
    tokenizer = None
    max_ngram_size = 1
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,1,1)
    # save to DB
    tbl_name = 'clef_2010_{0}_all_unigrams_stopwords'.format(vocabulary_src)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_unigrams_stopwords_vocabulary(corpus,stop_words,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,1,1)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_unigrams_stopwords'.format(vocabulary_src)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_bigrams_stopwords_vocabulary(corpus,stop_words,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,1,1)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_bigrams_stopwords'.format(vocabulary_src)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_lemmatized_unigrams_stopwords_vocabulary(corpus,stop_words,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_lemmas_unigrams_stopwords_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_bigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,1,1)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_bigrams'.format(vocabulary_src,1,1)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name    
...
def build_raw_lemmatized_bigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_lemmas_bigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name    
...
def build_raw_lemmatized_all_bigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_lemmas_all_bigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name    
...
def build_raw_stemmed_all_bigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_stems_all_bigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name    
...
def build_raw_lemmatized_test_bigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_lemmas_test_bigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name    
...
def build_raw_stemmed_test_bigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_stems_test_bigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name    
...
def build_bigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_bigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name    
...
def build_raw_stemmed_unigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_stems_unigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_stemmed_unigrams_stopwords_vocabulary(corpus,stop_words,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_stems_unigrams_stopwords_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_stemmed_bigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_stems_bigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_stemmed_bigrams_vocabulary(corpus,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_stems_bigrams_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build_raw_stemmed_bigrams_stopwords_vocabulary(corpus,stop_words,vocabulary_src):
    ...
    vocabulary = build_vocabulary(corpus,tokenizer,stop_words,max_ngram_size,min_df,min_tf)
    # save to DB
    tbl_name = 'clef_2010_{0}_raw_stems_bigrams_stopwords_df{1}_tf{2}'.format(vocabulary_src,min_df,min_tf)
    save_vocabulary(vocabulary,tbl_name)
    print 'done '+tbl_name
...
def build(vocabulary_src):
    ...
    #build_all_bigrams_vocabulary(corpus,vocabulary_src)
